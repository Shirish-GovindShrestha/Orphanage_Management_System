...
public class MergeSort {
    ...
    public static ArrayList<OrphanModel> mergeSort(ArrayList<OrphanModel> unsortedData, boolean isDesc, String attribute) {
        ...
        switch (attribute) {
            ...
            case "FirstName" -> {
                return mergeByFirstName(left, right, isDesc);
            }
            default -> {
                return null;
            }
     ...
     * @return the sorted list
     */
    public static ArrayList<OrphanModel> mergeByFirstName(ArrayList<OrphanModel> left, ArrayList<OrphanModel> right, boolean isDesc) {
        ArrayList<OrphanModel> merged = new ArrayList<>();
        int i = 0;
        int j = 0;
        ...
        while (i < left.size() && j < right.size()) {
            if (isDesc) {
                if (left.get(i).getFirstName().compareTo(right.get(j).getFirstName()) > 0) {
                    merged.add(left.get(i++));
                } else {
                    merged.add(right.get(j++));
                }
